...
figure('name','Small Image Histogram'), imhist(small_image(:,:,1));
figure('name', 'Original Image Histogram'), imhist(image(:,:,1));
...
figure('name','Rescaled Image Histogram'), imhist(rescaled_image(:,:,1));
figure('name','Original Image Histogram'), imhist(image(:,:,1));
...
subplot(2,2,1),imshow(square_image55), title('Box Filter 5x5');

h = ones(8,8)/64;
square_image88 = imfilter(image(:,:,1),h);
subplot(2,2,2), imshow(square_image88), title('Box Filter 8x8');

h = ones(15,15)/225;
square_image15 = imfilter(image(:,:,1),h);
subplot(2,2,3), imshow(square_image15), title('Box Filter 15x15');
...
figure('name','Histogram: Gaussian Filter: 10x10 o=8'), imhist(imagesigma810);
...
subplot(2,2,2),imshow(rgbfiltered), title('image smoothed by gaussian 10x10 o=8');
...
horizontal_smooth_averaged = imfilter(image(:,:,1),h);
...
subplot(2,2,1),imshow(horizontal_smooth), title('Horizontal Smooth');
subplot(2,2,2),imshow(vertical_smooth), title('Vertical Smooth');
subplot(2,2,3),imshow(square_smooth_not_averaged), title('5x5 Box Smooth');
...
subplot(2,2,1),imshow(horizontal_smooth3), title('Horizontal Smooth');
subplot(2,2,2),imshow(vertical_smooth3), title('Vertical Smooth');
subplot(2,2,3),imshow(square_smooth_not_averaged3), title('5x5 Box Smooth');
...
input('Press enter to show the histograms of a median 12x12 smoothed image and the original')
figure('name','image Histogram'), imhist(image(:,:,1))
figure('name','image Filtered with Median 3x3 histogram'), imhist(median_33)
figure('name','image Filtered with Median 8x8 histogram'), imhist(median_88)
figure('name','image Filtered with Median 12x12 histogram'), imhist(median_1212)
...
% impacting heavily the gray colors distribution, such as 3x3 filter.
