function [] = trainAndClassify(FEAT_FACE,FEAT_NON_FACE, I, x1, y1, maskInfo)
 %% Define the new regions of the test image 
    ...
    figure('name','Scatter plot for training (classified) vs test samples rectangular intensity differences');
    hold on
    ...
    input('Press enter to visualize the test samples');
    FEAT_FACE_TEST = FEAT_TEST(class==1,:);
    FEAT_NON_FACE_TEST = FEAT_TEST(class==0,:);
