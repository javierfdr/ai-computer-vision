...
figure, imhist(small_corals);
figure, imhist(corals);
...
input('Press enter to show three smoothed images by using square filters
of size 3x3, 5x5, 8x8');
h = ones(3,3)/9;
square_corals33 = imfilter(corals,h);
figure, imshow(square_corals33);

h = ones(5,5)/25;
square_corals55 = imfilter(corals,h);
figure, imshow(square_corals55);

h = ones(8,8)/64;
square_corals88 = imfilter(corals,h);
figure, imshow(square_corals88);

input('Press enter to show three smoothed images by using gaussian
filters of (sigma, mask) as (3, 3x3), (3, 5x5), (3, 8x8) ')
